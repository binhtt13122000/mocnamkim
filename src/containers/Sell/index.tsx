...
const Sell = () => {
                ...
                if (data) {
                    if (!data.id) {
                        ...
                        mutateCreate(
                            ...
                            {
                                onSuccess: (data) => {
                                    setValue("phone", data.insert_customer_one?.id || 0);
                                    showSnackbar({
    ...
    const removeRecord = (productId: number, total: number) => {
        setOrderDetails([...orderDetails].filter((x) => x.productid !== productId));
        showSnackbar({
    ...
    const addToOder = () => {
        ...
        if (orderDetails.findIndex((x) => x.productid === getValues("productCode")) !== -1) {
            showSnackbar({
                children: "Đã thêm vào hóa đơn. Xóa để nhập lại",
                severity: "error",
            });
            return;
        }
                                    ...
                                    <TableRow key={x.productid}>
                                        ...
                                        <TableCell>
                                            <Button
                                                size="small"
                                                color="error"
                                                onClick={() => removeRecord(x.productid, x.total)}
                                            >
                                                Xóa
                                            </Button>
                                        </TableCell>
